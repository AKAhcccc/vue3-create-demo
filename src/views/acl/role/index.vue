<template>
  <div>
    ...
    <el-cart>
      <el-form :inline="true" :model="format" class="form">
        <el-form-item label="职位搜索">
          <el-input placeholder="请你输入搜索的关键字" v-model="keyword"></el-input>
        </el-form-item>
        <el-form-item label="">
          <el-button type="primary" size="default" @click="Search">搜索</el-button>
          <el-button type="primary" size="default" @click="refsh">重置</el-button>
        </el-form-item>
      </el-form>
    </el-cart>
    <!-- 身体table展示数据部分 -->
    <el-cart>
      ...
      <el-table border style="margin: 10px 0;" :data="allRole">
        ...
        <el-table-column label="操作" width="300px" align="center">
          ...
          <template #="{ row }">
            <div>
              <el-button type="primary" size="small" icon="UserFilled" @click="setPermission(row)">分配权限</el-button>
    ...
    <!-- 添加职位部分，对话框 -->
    <el-dialog :title="RoleParams.id ? '更新职位' : '新增职位'" v-model="dialogVisited" ref="form">
      ...
      <template #footer>
        <span class="dialog-footer">
          <el-button type="primary" size="default" @click="dialogVisited = false">取消</el-button>
...
import { reqRoleAllList, reqRoleAdd, reqAllTreeRoles, reqSetPermission, reqReoveRole } from '@/api/aci/role/index';
import type { RoleResponseData, Records, RoleData, MenuResponseData, TreeList } from '@/api/aci/role/type'
// 当前页码
...
const getRoleList = async (pager = 1) => {
  pageNo.value = pager
  let result: RoleResponseData = await reqRoleAllList(pageNo.value, pageSize.value, keyword.value);
...
const save = async () => {
  ...
  if (result.code === 200) {
    ElMessage({ type: 'success', message: RoleParams.id ? '更新数据成功' : '新增数据成功' })
    dialogVisited.value = false
    // 更新完毕，再次获取数据
    getRoleList(RoleParams.id ? pageNo.value : 1)

  } else {
    ElMessage({ type: 'success', message: RoleParams.id ? '更新数据失败' : '新增数据失败' })
  }
}
